...
const PresentAuditModal = ({
  handleCloseSecModalPresent,
  handleConfirmSetMinusPresentCounter,
  handleConfirmSetActiveToTrue,
}) => {
  const handleCloseSet = async () => {
    try {
      await handleConfirmSetMinusPresentCounter();
      await handleConfirmSetActiveToTrue();
    } catch (err) {
      console.error(err);
    } finally {
      try {
        await handleCloseSecModalPresent();
      } catch (err) {
        console.error(err);
      }
    }
...
export default PresentAuditModal;
